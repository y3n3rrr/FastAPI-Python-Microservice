...
class ApiTests(unittest.TestCase):
    ...
    def test_checkout_flow_and_inventory_update(self) -> None:
        ...
        brand_create = self.client.post(
            ...
            json={
                "name": "Checkout Brand",
                "slug": "checkout-brand",
                "description": "Brand for checkout tests",
                "logo_url": "https://example.com/checkout-brand.png",
                "is_active": True,
            },
            headers=auth_headers,
        )
        ...
        category_create = self.client.post(
            ...
            json={
                ...
                "description": "Category for checkout tests",
                "is_active": True,
            },
            headers=auth_headers,
        )
        ...
        product_create = self.client.post(
            ...
            json={
                "category_id": category_id,
                "brand_id": brand_id,
                "name": "Checkout Product 1L",
                "slug": "checkout-product-1l",
        ...
        variant_create = self.client.post(
            ...
            json={
                "product_id": product_id,
                "sku": "SKU-CHECKOUT-001",
                "barcode": "8690000000301",
                "name": "1L",
                "color": None,
                "size": "1L",
                "price": "39.90",
        ...
        payment_method_create = self.client.post(
            "/payment-methods",
            json={
                "user_id": user_id,
                "provider": "stripe",
                "provider_customer_id": "cus_checkout_001",
                "provider_payment_method_id": "pm_checkout_001",
                "card_brand": "visa",
                "card_last4": "4242",
                "exp_month": 12,
                "exp_year": 2030,
        ...
        self.assertEqual(payload["payment_intent"]["status"], "authorized")
        self.assertEqual(payload["order"]["status"], "confirmed")
        self.assertEqual(payload["order"]["total_amount"], "119.70")
    ...
    def test_assistant_chat_flow(self) -> None:
        ...
        register_response = self.client.post(
            ...
            json={
                "name": "Assistant",
                "surname": "Tester",
                "email": "assistant-tester@example.com",
                "password": "supersecure",
                "is_active": True,
            },
        )
        ...
        first_chat = self.client.post(
            "/assistant/chat",
            json={"user_id": user_id, "message": "How are you?"},
            headers=auth_headers,
        )
        self.assertEqual(first_chat.status_code, 200)
        first_payload = first_chat.json()
        session_id = first_payload["session"]["id"]
        self.assertEqual(first_payload["user_message"]["role"], "user")
        self.assertEqual(first_payload["assistant_message"]["role"], "assistant")
        self.assertIn("How are you?", first_payload["assistant_message"]["content"])

        second_chat = self.client.post(
            "/assistant/chat",
            json={"user_id": user_id, "session_id": session_id, "message": "Tell me a joke"},
            headers=auth_headers,
        )
